...
/*
====================================================================
MUSE BRAINWAVE HARMONIC EXPLORER V10
====================================================================
...
instr 5  ; Jazz chord synth with FIXED sensor assignment
	...
	if kFirstNote == 1 then
		printks "\n>>> V10 - EXOTIC EEG HARMONIC EXPLORER\n", 0
	...
	; Three alternate orchestras.
	if gkOrchestraMode == 0 then
		; Classic sine ensemble inherited from V9.
		aout1L_a oscili 0.3 * gkSensor1Vol * kGate1, kFreq1L * (icps/440) * kdetune
		aout1L_b oscili 0.3 * gkSensor1Vol * kGate1, kFreq1L * (icps/440) * 1.002
		aout1L = aout1L_a + aout1L_b
		aout2L oscili 0.4 * gkSensor2Vol * kGate2, kFreq2L * (icps/440) * kdetune
		aout3L oscili 0.4 * gkSensor3Vol * kGate3, kFreq3L * (icps/440) * kdetune
		aout4L oscili 0.4 * gkSensor4Vol * kGate4, kFreq4L * (icps/440) * kdetune
		aout1R_a oscili 0.3 * gkSensor1Vol * kGate1, kFreq1R * (icps/440) * kdetune
		aout1R_b oscili 0.3 * gkSensor1Vol * kGate1, kFreq1R * (icps/440) * 0.999
		aout1R = aout1R_a + aout1R_b
		aout2R oscili 0.4 * gkSensor2Vol * kGate2, kFreq2R * (icps/440) * kdetune
		aout3R oscili 0.4 * gkSensor3Vol * kGate3, kFreq3R * (icps/440) * kdetune
		aout4R oscili 0.4 * gkSensor4Vol * kGate4, kFreq4R * (icps/440) * kdetune
	elseif gkOrchestraMode == 1 then
		; Glass additive: each voice has a different upper-partial emphasis.
		aout1L oscili 0.28 * gkSensor1Vol * kGate1, kFreq1L * (icps/440)
		a1pL oscili 0.12 * gkSensor1Vol * kGate1, kFreq1L * (icps/440) * 2.01
		aout1L += a1pL
		aout2L oscili 0.24 * gkSensor2Vol * kGate2, kFreq2L * (icps/440) * 1.001
		a2pL oscili 0.16 * gkSensor2Vol * kGate2, kFreq2L * (icps/440) * 3.01
		aout2L += a2pL
		aout3L oscili 0.22 * gkSensor3Vol * kGate3, kFreq3L * (icps/440) * 0.999
		a3pL oscili 0.14 * gkSensor3Vol * kGate3, kFreq3L * (icps/440) * 4.02
		aout3L += a3pL
		aout4L oscili 0.2 * gkSensor4Vol * kGate4, kFreq4L * (icps/440) * 1.002
		a4pL oscili 0.16 * gkSensor4Vol * kGate4, kFreq4L * (icps/440) * 5.01
		aout4L += a4pL
		aout1R oscili 0.28 * gkSensor1Vol * kGate1, kFreq1R * (icps/440) * 0.999
		a1pR oscili 0.12 * gkSensor1Vol * kGate1, kFreq1R * (icps/440) * 2.02
		aout1R += a1pR
		aout2R oscili 0.24 * gkSensor2Vol * kGate2, kFreq2R * (icps/440) * 1.002
		a2pR oscili 0.16 * gkSensor2Vol * kGate2, kFreq2R * (icps/440) * 3.02
		aout2R += a2pR
		aout3R oscili 0.22 * gkSensor3Vol * kGate3, kFreq3R * (icps/440)
		a3pR oscili 0.14 * gkSensor3Vol * kGate3, kFreq3R * (icps/440) * 4.01
		aout3R += a3pR
		aout4R oscili 0.2 * gkSensor4Vol * kGate4, kFreq4R * (icps/440) * 1.001
		a4pR oscili 0.16 * gkSensor4Vol * kGate4, kFreq4R * (icps/440) * 5.02
		aout4R += a4pR
	else
		; Dark hybrid: sub/fundamental plus beating fifths and low-pass smoothing.
		aout1L oscili 0.36 * gkSensor1Vol * kGate1, kFreq1L * (icps/440) * 0.5
		aout2L oscili 0.28 * gkSensor2Vol * kGate2, kFreq2L * (icps/440)
		aout3L oscili 0.24 * gkSensor3Vol * kGate3, kFreq3L * (icps/440) * 1.5
		aout4L oscili 0.22 * gkSensor4Vol * kGate4, kFreq4L * (icps/440) * 2.0
		aout1R oscili 0.36 * gkSensor1Vol * kGate1, kFreq1R * (icps/440) * 0.5
		aout2R oscili 0.28 * gkSensor2Vol * kGate2, kFreq2R * (icps/440) * 1.001
		aout3R oscili 0.24 * gkSensor3Vol * kGate3, kFreq3R * (icps/440) * 1.498
		aout4R oscili 0.22 * gkSensor4Vol * kGate4, kFreq4R * (icps/440) * 2.002
		aout1L tone aout1L, 900
		aout2L tone aout2L, 1400
		aout3L tone aout3L, 1800
		aout4L tone aout4L, 2200
		aout1R tone aout1R, 900
		aout2R tone aout2R, 1400
		aout3R tone aout3R, 1800
		aout4R tone aout4R, 2200
	endif
...
instr 6  ; Safe melodic generator (controlled by Gamma) - uses schedkwhen
	...
	kRate = (0.08 + (gkMelodyActivity * 1.2) + (kBetaActivity * 0.5)) * kRateMult * (0.65 + gkMelodyComplexity * 1.4)
...
instr 10 ; Delta-driven walking bass generator
	...
	kThetaActivity = limit((kThetaMean + 2.5) / 4.0, 0, 1)
	kBetaActivity = limit((kBetaMean + 2.5) / 4.0, 0, 1)
	kBassRate = limit(0.55 + (kDeltaActivity * 0.85) + (kBetaActivity * 0.25), 0.35, 1.6)
	...
	if ktrig == 1 then
		...
		kAmp = 0.022 + (kDeltaActivity * 0.035)
		schedkwhen 1, 0, 0, 11, 0, kDur, kBassNote, kAmp
		
		if gkPrintToggle == 1 then
			printks ">>> BASS rate %.2f step %d note %.0f amp %.3f delta %.3f theta %.3f beta %.3f\n", 0, kBassRate, kStep, kBassNote, kAmp, kDeltaActivity, kThetaActivity, kBetaActivity
		endif
	endif
done:
endin
...
instr 11 ; Bass note voice
	iNote = p4
	iAmp = p5
	kEnv madsr 0.015, 0.08, 0.45, 0.18
	kFreq = cpsmidinn(iNote)
	aSub oscili iAmp * 0.18, kFreq * 0.5
	aFund oscili iAmp * 0.85, kFreq
	aClick oscili iAmp * 0.12, kFreq * 2
	aBass = (aSub + aFund + aClick) * kEnv
	aBass tone aBass, 650
	garvbL += aBass * 0.08
	garvbR += aBass * 0.08
	gadelL += aBass * 0.04
	gadelR += aBass * 0.04
